...
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, AreaChart, Area, BarChart, Bar, PieChart, Pie, Cell, ReferenceLine, Label, LabelList } from 'recharts';
...
const BarChartLabel = (props) => {
    ...
    if (value > 0) {
        return (
            <text x={x + width / 2} y={y} dy={-4} fill="#666" fontSize={11} textAnchor="middle">
...
const LineChartLabel = (props) => {
    const { x, y, value, stroke } = props;
    if (value === 0 || value === undefined) return null;
    return (
        <g transform={`translate(${x},${y})`}>
            <rect x={-20} y={-22} width={40} height={18} rx={4} ry={4} fill="rgba(255, 255, 255, 0.7)" stroke={stroke} strokeWidth={1} />
            <text x={0} y={-13} textAnchor="middle" dominantBaseline="middle" fill={stroke} fontSize={10} fontWeight="bold">
                {value.toLocaleString()}
            </text>
        </g>
    );
...
const CnChartsCard = ({ status, apiData, errorMessage, onRetry, dateRangeString, byCountConfig, byPackConfig, byPieceConfig, byBahtConfig }) => {
    ...
    const renderContent = () => {
        ...
        if (status === 'empty') { return <DataMessage status="info" message="ไม่พบข้อมูลในข่วงวันที่ที่เลือก" />; }
                ...
                case 'line': return ( 
                    <ChartWrapper>
                        <LineChart data={chartData} margin={{ top: 20, right: 20, left: -10, bottom: 25 }}>
                            ...
                            <Legend content={<CustomLegend chartType="line" theme={theme} colors={{...colors, COLOR_TARGET}} legendItems={legendItems}/>} />
                            {unit === 'ใบ' && (<ReferenceLine y={TARGET_LINE_VALUE} stroke={COLOR_TARGET} strokeWidth={2} strokeDasharray="3 3"><Label value={`เป้าหมาย: ${TARGET_LINE_VALUE}`} position="insideTopRight" fill={COLOR_TARGET} fontSize={12} fontWeight="bold"/></ReferenceLine>)}
                            
                            <Line type="monotone" dataKey={dataKeys.total} stroke={theme.palette.primary.main} strokeWidth={2} activeDot={{ r: 8 }} isAnimationActive={!showLabels} >
                                {/* **[FIX]** เปลี่ยน dataKey ให้ถูกต้อง */}
                                {showLabels && <LabelList dataKey={dataKeys.total} content={<LineChartLabel stroke={theme.palette.primary.main} />} />}
                            </Line>
                            <Line type="monotone" dataKey={dataKeys.type43} stroke={colors.type43} isAnimationActive={!showLabels} >
                                {/* **[FIX]** เปลี่ยน dataKey ให้ถูกต้อง */}
                                {showLabels && <LabelList dataKey={dataKeys.type43} content={<LineChartLabel stroke={colors.type43} />} />}
                            </Line>
                            <Line type="monotone" dataKey={dataKeys.type42} stroke={colors.type42} isAnimationActive={!showLabels} >
                                {/* **[FIX]** เปลี่ยน dataKey ให้ถูกต้อง */}
                                {showLabels && <LabelList dataKey={dataKeys.type42} content={<LineChartLabel stroke={colors.type42} />} />}
                            </Line>
                        </LineChart>
                    </ChartWrapper> 
                );
                case 'area': return ( <ChartWrapper><AreaChart data={chartData} margin={{ top: 20, right: 20, left: -10, bottom: 25 }}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="date" /><YAxis tickFormatter={yAxisFormatter} /><Tooltip content={<CustomChartTooltip unit={unit} />} /><Legend content={<CustomLegend chartType="area" theme={theme} colors={colors} legendItems={legendItems}/>} /><Area type="monotone" dataKey={dataKeys.total} stroke={theme.palette.primary.dark} fill={theme.palette.primary.light} fillOpacity={0.3} label={showLabels} /><Area type="monotone" dataKey={dataKeys.type43} stackId="1" stroke={colors.type43} fill={colors.type43} fillOpacity={0.6} label={showLabels} /><Area type="monotone" dataKey={dataKeys.type42} stackId="1" stroke={colors.type42} fill={colors.type42} fillOpacity={0.6} label={showLabels} /></AreaChart></ChartWrapper> );
                case 'bar': return ( <ChartWrapper><BarChart data={chartData} margin={{ top: 20, right: 20, left: -10, bottom: 25 }}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="date" /><YAxis tickFormatter={yAxisFormatter} /><Tooltip content={<CustomChartTooltip unit={unit} />} /><Legend content={<CustomLegend chartType="bar" theme={theme} colors={colors} legendItems={legendItems}/>} /><Bar dataKey={dataKeys.total} fill={theme.palette.primary.main}>{showLabels && <LabelList dataKey={dataKeys.total} content={<BarChartLabel />} />}</Bar><Bar dataKey={dataKeys.type43} fill={colors.type43}>{showLabels && <LabelList dataKey={dataKeys.type43} content={<BarChartLabel />} />}</Bar><Bar dataKey={dataKeys.type42} fill={colors.type42}>{showLabels && <LabelList dataKey={dataKeys.type42} content={<BarChartLabel />} />}</Bar></BarChart></ChartWrapper> );
...
//             switch (chartComponent) {
//                 case 'line': return ( <ChartWrapper><LineChart data={chartData} margin={{ top: 20, right: 20, left: -10, bottom: 25 }}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="date" /><YAxis tickFormatter={yAxisFormatter} /><Tooltip content={<CustomChartTooltip unit={unit} />} /><Legend content={<CustomLegend chartType="line" theme={theme} colors={{...colors, COLOR_TARGET}} legendItems={legendItems}/>} />{unit === 'ใบ' && (<ReferenceLine y={TARGET_LINE_VALUE} stroke={COLOR_TARGET} strokeWidth={2} strokeDasharray="3 3"><Label value={`เป้าหมาย: ${TARGET_LINE_VALUE}`} position="insideTopRight" fill={COLOR_TARGET} fontSize={12} fontWeight="bold"/></ReferenceLine>)}<Line type="monotone" dataKey={dataKeys.total} stroke={theme.palette.primary.main} strokeWidth={2} activeDot={{ r: 8 }} isAnimationActive={!showLabels} label={showLabels} /><Line type="monotone" dataKey={dataKeys.type43} stroke={colors.type43} isAnimationActive={!showLabels} label={showLabels} /><Line type="monotone" dataKey={dataKeys.type42} stroke={colors.type42} isAnimationActive={!showLabels} label={showLabels} /></LineChart></ChartWrapper> );
//                 case 'area': return ( <ChartWrapper><AreaChart data={chartData} margin={{ top: 20, right: 20, left: -10, bottom: 25 }}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="date" /><YAxis tickFormatter={yAxisFormatter} /><Tooltip content={<CustomChartTooltip unit={unit} />} /><Legend content={<CustomLegend chartType="area" theme={theme} colors={colors} legendItems={legendItems}/>} /><Area type="monotone" dataKey={dataKeys.total} stroke={theme.palette.primary.dark} fill={theme.palette.primary.light} fillOpacity={0.3} label={showLabels} /><Area type="monotone" dataKey={dataKeys.type43} stackId="1" stroke={colors.type43} fill={colors.type43} fillOpacity={0.6} label={showLabels} /><Area type="monotone" dataKey={dataKeys.type42} stackId="1" stroke={colors.type42} fill={colors.type42} fillOpacity={0.6} label={showLabels} /></AreaChart></ChartWrapper> );
//                 case 'bar': return ( <ChartWrapper><BarChart data={chartData} margin={{ top: 20, right: 20, left: -10, bottom: 25 }}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="date" /><YAxis tickFormatter={yAxisFormatter} /><Tooltip content={<CustomChartTooltip unit={unit} />} /><Legend content={<CustomLegend chartType="bar" theme={theme} colors={colors} legendItems={legendItems}/>} /><Bar dataKey={dataKeys.total} fill={theme.palette.primary.main}>{showLabels && <LabelList dataKey="count" content={<BarChartLabel />} />}</Bar><Bar dataKey={dataKeys.type43} fill={colors.type43}>{showLabels && <LabelList dataKey="count" content={<BarChartLabel />} />}</Bar><Bar dataKey={dataKeys.type42} fill={colors.type42}>{showLabels && <LabelList dataKey="count" content={<BarChartLabel />} />}</Bar></BarChart></ChartWrapper> );
//                 case 'pie': return ( <ChartWrapper><PieChart margin={{ top: 5, right: 20, left: -10, bottom: 25 }}><Pie data={pieData} cx="50%" cy="50%" labelLine={false} outerRadius={120} fill="#8884d8" dataKey="value" label={({ name, percent }) => `${name.replace(` (${unit})`, '')} ${(percent * 100).toFixed(0)}%`} >{pieData.map((entry, index) => ( <Cell key={`cell-${index}`} fill={PIE_COLORS[index % PIE_COLORS.length]} /> ))}</Pie><Tooltip content={<CustomPieTooltip />} /><Legend content={<CustomLegend chartType="pie" theme={theme} colors={colors} legendItems={legendItems}/>} /></PieChart></ChartWrapper> );
//                 default: return null;
//             }
//         }
